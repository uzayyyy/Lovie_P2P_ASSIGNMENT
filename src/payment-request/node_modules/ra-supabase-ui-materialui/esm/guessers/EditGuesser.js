...
import { EditView, Loading, } from 'ra-ui-materialui';
...
export var EditGuesserView = function (props) {
    ...
    var _b = React.useState(null), child = _b[0], setChild = _b[1];
    if (!resource) {
    ...
    React.useEffect(function () {
        ...
        var resourceDefinition = (_a = schema.definitions) === null || _a === void 0 ? void 0 : _a[resource];
        var requiredFields = (resourceDefinition === null || resourceDefinition === void 0 ? void 0 : resourceDefinition.required) || [];
        if (!resourceDefinition || !resourceDefinition.properties) {
            throw new Error("The resource ".concat(resource, " is not defined in the API schema"));
        }
        var inferredInputs = Object.keys(resourceDefinition.properties)
            .filter(function (source) { return source !== 'id'; })
            .filter(function (source) {
            return resourceDefinition.properties[source].format !== 'tsvector';
        })
            .map(function (source) {
        ...
        var inferredForm = new InferredElement(editFieldTypes.form, null, inferredInputs);
        setChild(inferredForm.getElement());
        if (!enableLog)
            return;
        var representation = inferredForm.getRepresentation();
        var components = ['Edit']
            .concat(Array.from(new Set(Array.from(representation.matchAll(/<([^/\s>]+)/g))
            .map(function (match) { return match[1]; })
            .filter(function (component) { return component !== 'span'; }))))
            .sort();
        var warnings = inferredInputs
            .map(function (inferredInput) { return inferredInput.getWarning(); })
            .filter(function (warning) { return warning != null; });
        // eslint-disable-next-line no-console
        console.log("Guessed Edit:\n            \nimport { ".concat(components.join(', '), " } from 'react-admin';\n            \nexport const ").concat(capitalize(singularize(resource)), "Edit = () => (\n    <Edit>\n").concat(representation, "\n    </Edit>\n);").concat(warnings.length > 0
            ? warnings.map(function (warning) { return "\n\n".concat(warning); }).join('')
            : ''));
